...
function grabPhotos(database, fsrd, res, rej) {
  fetch(`${apiUrl}/${fsrd.queryName}/photos?sol=${fsrd.maxSol}&api_key=${apiKey}`)
    .then(resp => resp.json())
    .then(json => {
      ...
      database.collection("dates").updateOne(
        { id: roverId },
        { $set: formattedData },
        { upsert: true },
        function (err, result) {
...
getRovers().then(resp => {
  ...
  Mongo.connect(mgurl, {
    ...
    const promises = fetchSpecificRoverData.map(fsrd => {
      console.log("starting the photo storing process with", fsrd.queryName)
      return new Promise((res, rej) => {
        grabPhotos(dbo, fsrd, res, rej);
      });
    });
